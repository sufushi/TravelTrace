...
import com.rdc.project.traveltrace.utils.player.InterData;
...
public class ControllerCover extends BaseCover implements OnTimerUpdateListener, OnTouchGestureListener, View.OnClickListener {
    ...
    public ControllerCover(Context context, boolean isPreview) {
        super(context);
    ...
    protected void onCoverAttachedToWindow() {
        ...
        DataSource dataSource = getGroupValue().get(InterData.Key.KEY_DATA_SOURCE);
        setTitle(dataSource);

        boolean topEnable = getGroupValue().getBoolean(InterData.Key.KEY_CONTROLLER_TOP_ENABLE, false);
        mControllerTopEnable = topEnable;
        if (!topEnable) {
            setTopContainerState(false);
        }

        boolean screenSwitchEnable = getGroupValue().getBoolean(InterData.Key.KEY_CONTROLLER_SCREEN_SWITCH_ENABLE, true) && !mIsPreview;
    ...
    private IReceiverGroup.OnGroupValueUpdateListener mOnGroupValueUpdateListener =
            new IReceiverGroup.OnGroupValueUpdateListener() {
                ...
                public String[] filterKeys() {
                    return new String[]{
                            InterData.Key.KEY_COMPLETE_SHOW,
                            InterData.Key.KEY_TIMER_UPDATE_ENABLE,
                            InterData.Key.KEY_DATA_SOURCE,
                            InterData.Key.KEY_IS_LANDSCAPE,
                            InterData.Key.KEY_CONTROLLER_TOP_ENABLE};
                }

                @Override
                public void onValueUpdate(String key, Object value) {
                    if (key.equals(InterData.Key.KEY_COMPLETE_SHOW)) {
                        boolean show = (boolean) value;
                        if (show) {
                            setControllerState(false);
                        }
                        setGestureEnable(!show);
                    } else if (key.equals(InterData.Key.KEY_CONTROLLER_TOP_ENABLE)) {
                        mControllerTopEnable = (boolean) value;
                        if (!mControllerTopEnable) {
                            setTopContainerState(false);
                        }
                    } else if (key.equals(InterData.Key.KEY_IS_LANDSCAPE)) {
                        setSwitchScreenIcon((Boolean) value);
                    } else if (key.equals(InterData.Key.KEY_TIMER_UPDATE_ENABLE)) {
                        mTimerUpdateProgressEnable = (boolean) value;
                    } else if (key.equals(InterData.Key.KEY_DATA_SOURCE)) {
    ...
    public void onPlayerEvent(int eventCode, Bundle bundle) {
        switch (eventCode) {
            case OnPlayerEventListener.PLAYER_EVENT_ON_DATA_SOURCE_SET:
                ...
                getGroupValue().putObject(InterData.Key.KEY_DATA_SOURCE, data);
                setTitle(data);
                break;
    ...
    public Bundle onPrivateEvent(int eventCode, Bundle bundle) {
        switch (eventCode) {
            case InterData.PrivateEvent.EVENT_CODE_UPDATE_SEEK:
                if (bundle != null) {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.cover_player_controller_image_view_back_icon:
                notifyReceiverEvent(InterData.Event.EVENT_CODE_REQUEST_BACK, null);
                break;
            ...
            case R.id.cover_player_controller_image_view_switch_screen:
                notifyReceiverEvent(InterData.Event.EVENT_CODE_REQUEST_TOGGLE_SCREEN, null);
                break;
        }
    }
}
