...
public class DegreeSeekBar extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mIndicatorPath.moveTo(w / 2.0f, h / 2.0f + mFontMetrics.top / 2.0f - 18);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mPointCount; i++) {
            ...
            canvas.drawPoint(mCanvasClipBounds.centerX() + (i - mPointCount / 2.0f) * mPointMargin,
        ...
        if (mCurrentDegrees >= 10) {
            canvas.drawText(mCurrentDegrees + suffix, getWidth() / 2.0f - mTextWidths[0], mBaseLine, mTextPaint);
        } else if (mCurrentDegrees <= -10) {
            canvas.drawText(mCurrentDegrees + suffix, getWidth() / 2.0f - mTextWidths[0] / 2 * 3, mBaseLine, mTextPaint);
        } else if (mCurrentDegrees < 0) {
            canvas.drawText(mCurrentDegrees + suffix, getWidth() / 2.0f - mTextWidths[0], mBaseLine, mTextPaint);
        } else {
            canvas.drawText(mCurrentDegrees + suffix, getWidth() / 2.0f - mTextWidths[0] / 2, mBaseLine, mTextPaint);
        }
    ...
    private void drawDegreeText(int degrees, Canvas canvas, boolean canReach) {
        ...
        if (degrees == 0) {
            ...
            canvas.drawText("0°", getWidth() / 2.0f - mTextWidths[0] / 2 - mCurrentDegrees / 2.0f * mPointMargin,
                    getHeight() / 2.0f - 10,
                    mTextPaint);
        } else {
            canvas.drawText(degrees + suffix,
                    getWidth() / 2.0f + mPointMargin * degrees / 2 - mTextWidths[0] / 2 * 3 - mCurrentDegrees / 2.0f * mPointMargin,
                    getHeight() / 2.0f - 10,
                    mTextPaint);
        }
    }
